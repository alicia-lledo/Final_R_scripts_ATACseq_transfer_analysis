...
read_fragments_annotated <- read.delim("blood_bwa/blood_bwa_annotated_fragments.bed", header=FALSE)
#/well/jknight/ATACseq/ATACseq_001/pipeline_bwa_unprocessed_Nature_data/nature_unprocessed_bwa_annotated_fragments.bed
...
table.TssAFlnk <- table(subset(read_fragments_annotated, V3 == "TssFlnk")$V2)
...
labels <- rep("", 750) # or up to 750
labels[seq(50, 750, 50)] <- seq(50, 750, 50) # or up to 750

  
  c(rep("",49),"50",rep("",49),"100", rep("",49),"150", rep("",49),"200",rep("",49),"250",
...
par(oma=c(4,0,4,10)) 
heatmap.2(transposed.log10.normalised.matrix[,50:750], labCol = labels[50:750], col=newcolours, scale="row", 
          trace="none", key.xlab = "",cexRow=1.5, cexCol=1.5,srtCol=45,
          Colv = FALSE, Rowv = FALSE, dendrogram = "none", na.rm=TRUE,
          density.info="none",
          key.title ="Enrichment",keysize = 0.8) #remove the rows where the NA are

pdf("heatmap_CD14_jknight.pdf", width=20, height=10)
par(oma=c(4,0,4,10)) 
heatmap.2(transposed.log10.normalised.matrix[,0:750], labCol = labels[0:750], col=newcolours,scale="row", 
          trace="none", key.xlab = "",cexRow=1.5, cexCol=1.5,srtCol=45,
          Colv = FALSE, Rowv = FALSE, dendrogram = "none", na.rm=TRUE,
          density.info="none",
          key.title ="Enrichment",keysize = 0.8) #remove the rows where the NA are
...
#Heatmap arguments
#cexrRow and cexCol label size, relative to one (default)
  #strRow and strCol for labels orientation being 0 horizontal and having to indicate the degrees relative to it 
    #e.g vertical will be 180
  #key.xlab = "" to remove legend title
  #par(oma=c(2,2,4,4)) to modify margins of the plot

#pdf
  #To create the pdf I had to adjust width and height of the pdf
  #Also par function oma c(bottom, left, top, right) needed to be adjusted
